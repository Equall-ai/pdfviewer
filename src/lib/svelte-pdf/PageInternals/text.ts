...
export const SPECIAL_CHARS_REG_EXP = /([.*+?^${}()|[\]\\])|(\p{P})|(\s+)|(\p{M})|(\p{L})/gu;

let noSyllablesRegExp: RegExp;
let withSyllablesRegExp: RegExp;
...
function getNormalizeWithNFKC() {
  ...
  if (ranges.join('') !== NormalizeWithNFKC) {
    ...
    const diff = [...rangesStr].filter((c, i) => c !== NormalizeWithNFKC.charAt(i));
    if (diff.length === 1 && diff[0].trim() === '') {
      // TODO(ariel): this is a hack to fix the issue with the `NormalizeWithNFKC` string.
      // we need to remove this and trace the original issue when we reintegrate search.
...
export function getOriginalIndex(diffs: [number, number][], pos: number, len: number) {
  if (!diffs) {
    return [pos, len];
  }
  ...
  const end = pos + len - 1;
  let i = binarySearchFirstItem(diffs, (x: number[]) => x[0] >= start);
  if (diffs[i][0] > start) {
    --i;
  }

  let j = binarySearchFirstItem(diffs, (x: number[]) => x[0] >= end, i);
  if (diffs[j][0] > end) {
    --j;
  }
